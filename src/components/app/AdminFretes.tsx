...
const AdminFretes = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (description.trim().length < 3) {
      toast({ variant: "destructive", title: "Descrição inválida", description: "A descrição precisa ter ao menos 3 caracteres." });
      return;
    }
    await freightMutation.mutateAsync({ amount: parsedAmount, description: description.trim() });
  };
  ...
  return (
    <div className="p-4 space-y-4 pb-24">
        ...
        <CardContent>
          <form className="space-y-3" onSubmit={handleSubmit}>
            <div className="space-y-1">
              ...
              <Input id="freight-amount" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" />
            </div>
            <div className="space-y-1">
              <Label htmlFor="freight-description">Descrição</Label>
              <Input id="freight-description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ex.: Frete Raízen - São Paulo" />
            </div>
      ...
      {transactionsQuery.isLoading && <StatusBlock title="Carregando histórico" isLoading />}
      {transactionsQuery.isError && (
        <StatusBlock title="Erro ao buscar histórico" description={getFriendlyErrorMessage(toAppError(transactionsQuery.error))} onRetry={() => transactionsQuery.refetch()} />
      )}
